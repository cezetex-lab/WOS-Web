  ...
  return (
    <div onClick={onClick} className={`flex items-center justify-between p-3 bg-slate-900/40 rounded-xl border border-white/5 transition-all duration-200 ${onClick ? 'hover:border-white/10 hover:bg-slate-800/40 cursor-pointer active:scale-[0.99]' : ''}`}>
      <div className="flex-1 min-w-0">
        ...
        {date && <p className="text-[10px] text-slate-500 mt-0.5">{new Date(date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</p>}
      </div>
    ...
    <div>
      ...
      <div className="overflow-x-auto -mx-4 px-4">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-white/5">{columns.map(col => <th key={col.key} className="text-left text-[10px] font-bold text-slate-400 uppercase tracking-wider py-2 px-2">{col.label}</th>)}</tr></thead>
      ...
      {totalPages > 1 && <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/5"><span className="text-[10px] text-slate-500">{filtered.length} data • Hal {page + 1}/{totalPages}</span><div className="flex gap-1"><button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} className="px-2 py-1 rounded-lg text-[10px] font-bold bg-white/5 text-slate-400 disabled:opacity-30 hover:bg-white/10 transition-all">←</button><button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} className="px-2 py-1 rounded-lg text-[10px] font-bold bg-white/5 text-slate-400 disabled:opacity-30 hover:bg-white/10 transition-all">→</button></div></div>}
    </div>
  );
}
