...
export function ActionItem({ title, subtitle, date, badge, badgeType = 'warning', onClick }: ActionItemProps) {
  return (
...
export function EmptyState({ icon = '📭', title = 'Tidak ada data', subtitle, description, message }: EmptyStateProps) {
  const detail = subtitle ?? description ?? message;
  return (
...
/**
 * Cell renderer. Parameters are `any` on purpose: call sites pass strongly
 * typed row/cell helpers (e.g. `(v: string, row: LeaveRow) => ...`).
 */
export type ColumnRender = (value: any, row: any) => React.ReactNode;
...
export interface DataTableProps {
  columns?: Column[];
  /** Row objects of any shape — the table reads cells by column key. */
  data?: any[];
...
export function DataTable({ columns = [], data = [], searchPlaceholder = 'Cari...', onRowClick, emptyMessage = 'Tidak ada data', loading }: DataTableProps) {
    ...
    <div>
      ...
      <div className="overflow-x-auto -mx-4 px-4">
        <table className="w-full text-sm">
          ...
          <tbody>{paged.length === 0 ? <tr><td colSpan={columns.length} className="text-center py-8 text-slate-500 text-xs">{emptyMessage}</td></tr> : paged.map((row, i) => <tr key={String(row.id || i)} onClick={() => onRowClick?.(row)} className={`border-b border-white/3 transition-colors ${onRowClick ? 'cursor-pointer hover:bg-white/3 active:bg-white/5' : ''}`}>{columns.map(col => <td key={col.key} className="py-2.5 px-2 text-slate-300 text-xs">{col.render ? col.render(row[col.key], row) : String(row[col.key] ?? '')}</td>)}</tr>)}</tbody>
        </table>
      </div>
...
export function StatItem({ label, value, max = 100, color = '#38bdf8', suffix = '' }: StatItemProps) {
  const numValue = typeof value === 'number' ? value : 0;
  const pct = Math.min((numValue / Math.max(max, 1)) * 100, 100);
  return (
