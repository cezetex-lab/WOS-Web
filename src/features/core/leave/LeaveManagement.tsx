// ============================================================
// LeaveManagement.tsx — Custom Admin Leave Page
// RPC: admin_get_leave
// ============================================================
...
import { CardColor } from '@/lib/design-system/cards';
...
type LeaveResponse = LeaveRow[] | { data?: LeaveRow[] };
...
export default function LeaveManagement() {
  ...
  // ── COLUMNS ──
  const columns: Col<LeaveRow>[] = [
    {
      key: 'nama',
      label: 'Karyawan',
      render: (val, row) => (
        <div className="flex items-center gap-2">
          <Avatar name={String(val || row.nrp)} size="sm" />
    ...
    },
    {
      key: 'kuota',
      label: 'Kuota',
      render: (val) => <span className="text-xs font-semibold text-white">{Number(val || 0)} hari</span>,
    },
    {
      key: 'terpakai',
      label: 'Terpakai',
      render: (val, row) => {
        const terpakaiVal = Number(val || 0);
        const pct = row.kuota > 0 ? (terpakaiVal / row.kuota) * 100 : 0;
        return (
          <div className="flex items-center gap-2">
            <span className="text-xs text-orange-400">{terpakaiVal} hari</span>
    ...
    <PageLayout backTo="/admin" title="Manajemen Cuti" subtitle="Kuota & pemakaian cuti karyawan">
      ...
      <GlassCard accent="blue">
        <DataTable
          ...
          onRowClick={(row: any) => setSelected(row as LeaveRow)}
