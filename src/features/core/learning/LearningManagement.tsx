...
export default function LearningManagement() {
  ...
  const statuses = [...new Set(data.map(r => String(r.status || 'Enrolled')))];
  const filtered = tab === 'all' ? data : data.filter(r => String(r.status || 'Enrolled') === tab);
  ...
  const columns = [
    ...
    { key: 'program', label: 'Program', render: (v: unknown) => <span className="text-xs text-slate-300">{String(v || '-')}</span> },
    { key: 'category', label: 'Kategori', render: (v: unknown) => <Badge status={String(v || 'General')} type="info" /> },
    { key: 'progress', label: 'Progress', render: (v: unknown) => (
      <div className="w-full bg-slate-700 rounded-full h-2">
        <div className="bg-blue-500 h-2 rounded-full transition-all" style={{ width: `${Number(v) || 0}%` }} />
      </div>
    )},
    { key: 'status', label: 'Status', render: (v: unknown) => <Badge status={String(v || 'Enrolled')} type={v === 'Completed' ? 'success' : v === 'In Progress' ? 'warning' : 'info'} /> },
  ];
  ...
  return (
    <PageLayout backTo="/admin" title="📚 Learning Management" subtitle={`${data.length} peserta`}>
      ...
      {statuses.length > 1 && (
        <div className="mb-4">
          <Tabs tabs={[{ id: 'all', label: 'Semua', count: data.length }, ...statuses.map(s => ({ id: s, label: s, count: data.filter(r => String(r.status || 'Enrolled') === s).length }))]} active={tab} onChange={setTab} />
        </div>
      )}
