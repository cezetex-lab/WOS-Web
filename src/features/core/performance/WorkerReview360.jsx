...
export default function WorkerReview360() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const result = await rpc('get_reviews_360', { p_nrp: nrp });
      const d = result || {};
      setData(d.summary || {});
  ...
  return (
    <PageLayout backTo="/worker" title="🔄 360° Review Saya" subtitle={`${reviews.length} review dari rekan kerja`}>
      {/* My Average Scores */}
      <GlassCard accent="blue" className="mb-6">
        <h3 className="text-sm font-bold text-white mb-4 text-center">📊 Skor Rata-rata Saya</h3>
        ...
        <div className="text-center mt-4">
          <Badge status={`Overall: ${data.avg_overall || 0}`} type="blue" />
        </div>
      </GlassCard>

      {/* Reviews from others */}
      <GlassCard accent="teal">
        <h3 className="text-sm font-bold text-white mb-3">📝 Review dari Rekan Kerja</h3>
        <div className="space-y-2">
              ...
              <div className="grid grid-cols-4 gap-2 mb-2">
                {['leadership', 'communication', 'teamwork', 'innovation'].map(key => (
                  <div key={key} className="text-center">
                    ...
                    <p className={`text-xs font-bold ${getScoreColor(r[`${key}_score`] || 0)}`}>{r[`${key}_score`] || 0}</p>
                  </div>
                ))}
              </div>
          ...
          {reviews.length === 0 && <EmptyState title="Belum ada review 360° untuk Anda" icon="🔄" subtitle="Review akan muncul setelah rekan kerja memberikan penilaian" />}
        </div>
      </GlassCard>
    </PageLayout>
  );
