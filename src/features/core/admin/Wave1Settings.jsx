...
export default function Wave1Settings() {
  ...
  async function handleChangePassword(e) {
    ...
    setChanging(true);
    try {
      const result = await rpc('admin_change_password', {
        p_old_password: passwordForm.old,
  ...
  return (
    <PageLayout backTo="/admin" title="Pengaturan" subtitle="Konfigurasi sistem & keamanan">
      ...
      {/* ── PKWT EXPIRY ALERTS ── */}
      <SectionHeader title="⚠️ PKWT Expiry Alert" icon="📋" />
        ...
        <GlassCard title="Daftar PKWT" icon="👥" accent="orange" className="mb-6">
          {pkwtAlerts.map((a, i) => (
            <ActionItem
              key={i}
              title={`${a.nama} (${a.nrp})`}
              subtitle={`${a.posisi} • ${a.divisi} • Masuk: ${a.tanggal_masuk}`}
              date={a.expiry_date}
              badge={a.risk_level}
              badgeType={a.risk_level === 'EXPIRED' ? 'danger'
                : a.risk_level === 'CRITICAL' ? 'warning'
                : a.risk_level === 'WARNING' ? 'info' : 'success'}
            />
          ))}
        </GlassCard>
      )}
      ...
      <GlassCard title="Password Admin" icon="🔐" accent="blue" className="mb-6">
        <form onSubmit={handleChangePassword} className="space-y-4">
          ...
          <Button
            type="submit"
            color="teal"
            disabled={changing || !passwordForm.old || !passwordForm.new}
          >
      ...
      {/* ── SYSTEM INFO ── */}
      <SectionHeader title="ℹ️ System Info" icon="📊" />
      <GlassCard title="insightWOS" icon="📊" accent="teal">
        <div className="space-y-2 text-xs">
          <div className="flex justify-between">
            <span className="text-slate-400">Version</span>
            <span className="text-white font-semibold">v3.0.0</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-400">Branch</span>
            <span className="text-white font-semibold">migrasi-vite</span>
          </div>
          <div className="flex justify-between">
          ...
          <div className="flex justify-between">
            <span className="text-slate-400">RPC Functions</span>
            <span className="text-white font-semibold">65+</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-400">Features</span>
            <span className="text-white font-semibold">141 planned, 53+ implemented</span>
          </div>
        </div>
      </GlassCard>
    </PageLayout>
