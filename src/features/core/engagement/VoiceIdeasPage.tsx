...
export default function VoiceIdeasPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const result = await rpc('list_ideas') as Record<string, unknown>;
  ...
  const handleStatusUpdate = async (idea: Record<string, unknown>, newStatus: string) => {
    if (!isAdmin) return;
    try {
  ...
  const handleVote = async (idea: Record<string, unknown>) => {
    try {
      await rpc('vote_idea', { p_idea_id: idea.id });
      setData(data.map(r => r.id === idea.id ? { ...r, votes: (Number(r.votes) || 0) + 1 } : r));
    } catch (e) { }
  };

  const columns = [
    { key: 'nrp', label: 'NRP', render: (v: unknown) => <span className="text-xs font-mono text-slate-400">{String(v ?? '')}</span> },
    { key: 'title', label: 'Judul', render: (v: unknown) => <span className="text-sm font-semibold text-white">{(v as string) || '-'}</span> },
    { key: 'category', label: 'Kategori', render: (v: unknown) => <Badge status={(v as string) || 'Umum'} type="info" /> },
    { key: 'votes', label: 'Vote', render: (v: unknown) => <span className="text-sm font-bold text-yellow-400">👍 {String(v ?? 0)}</span> },
    { key: 'status', label: 'Status', render: (v: unknown) => <Badge status={(v as string) || 'New'} type={v === 'Implemented' ? 'success' : v === 'Review' ? 'warning' : v === 'Rejected' ? 'danger' : 'info'} /> },
  ];
  ...
  return (
    <PageLayout backTo={isAdmin ? '/admin' : '/worker'} title="💡 Ide & Voice" subtitle={`${data.length} ide masuk`}>
      <div className="grid grid-cols-3 gap-3 mb-6">
        ...
        <MetricCard icon="⭐" value={data.reduce((s: number, r) => s + ((r.votes as number) || 0), 0)} label="Total Vote" color="orange" />
      ...
      {showSubmit && (
        <GlassCard accent="blue" className="mb-4">
          ...
          <Input label="Judul" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="Judul ide Anda" />
          <Input label="Deskripsi" value={newDesc} onChange={(e) => setNewDesc(e.target.value)} placeholder="Jelaskan ide Anda" className="mt-2" />
      ...
      {statuses.length > 1 && (
        <div className="mb-4">
          <Tabs tabs={[{ id: 'all', label: 'Semua', count: data.length }, ...statuses.map(s => ({ id: String(s), label: String(s), count: data.filter(r => (r.status || 'New') === s).length }))]} active={tab} onChange={setTab} />
        </div>
      )}
      ...
      {selected && (
        <>
          ...
          <div className="fixed inset-x-0 bottom-0 z-50 max-h-[85vh] bg-slate-900 border-t border-white/10 rounded-t-3xl overflow-y-auto p-5 pb-8">
            ...
            <h2 className="text-lg font-bold text-white mb-2">{String(selected.title || 'Detail Ide')}</h2>
            <p className="text-xs text-slate-300 mb-4">{String(selected.description || selected.body || '-')}</p>
            {!isAdmin && (
