...
| TypeScript | ✅ DONE | — | 209 file TS total (157 `src` + 44 `tests` + 8 config), 0 tsc errors — `tsconfig` mencakup `src`+`tests`+`*.config.ts`, `allowJs: false`. Angka diverifikasi 2026-09-26 (Fix #2) dengan metodologi identik `tests/unit/doc-claims-vs-live.test.ts` (berkas ter-track; `src` = `.ts`+`.tsx`; `tests` = `.ts`+`.tsx`; config = file root `*.config.ts`) |
