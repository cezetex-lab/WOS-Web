...
│  DB: 209 tables, 671 functions, RLS on all tables          │
...
| TypeScript | ✅ DONE | — | 202 file TS total (157 `src` + 39 `tests` + 6 config), 0 tsc errors — `tsconfig` mencakup `src`+`tests`+`*.config.ts`, `allowJs: false` |
...
| Functions | 671 | 20 overloads (legacy renamed `_legacy_*`); +1 `ensure_attendance_partitions` (migration 225). LIVE 2026-09-19: 667 → **671** (+4 nama baru dari migrasi 231: 3 wrapper `verify_*_core` hasil RENAME + `auth_testing_override_bypass`) |
| Migrations tracked | 164 | Via `schema_migrations` (migration 219); 221–227 didaftarkan 2026-09-17 (§5.7 no.11); `008` + `229` + `230` diterapkan 2026-09-18 (`DITERAPKAN + terdaftar + checksum terverifikasi`). `231`–`233` diterapkan + terdaftar 2026-09-19. Rename sesi paralel 232–235 → 236–239 sudah terdaftar di live (236–239; nomor 232/233 dipakai file grant yang sudah ada), sehingga live kini 164 baris |
