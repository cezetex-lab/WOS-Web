...
| OPS-11 | **P3** | `get_branding` double-fire (2 panggilan paralel + re-fire) → churn networkidle. | Probe TASK 3 OPS-10: 4 calls, jeda 0ms/1588ms/8ms (`.agents/logs/task3-mill-churn.json`). | Dedupe in-flight/satu kali per sesi di layer pemanggil. Bukti: 1 call per mount. | OPEN |
| OPS-12 | **P3** | `useAdminAuth` navigate('/admin') saat role belum termuat → flash-redirect di halaman terlindungi. | `useAdminAuth.ts:21`. Probe TASK 3: admin_hrd di-redirect dari /admin/mill (URL akhir `/admin`, heading mill tak pernah muncul). | Tambah loading state sebelum navigate. Bukti: 4-page verify + E2E role-change PASS. | OPEN |
