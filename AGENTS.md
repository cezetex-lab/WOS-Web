...
|---|---|---|---|---|---|
| OPS-13 | **P3 (usul — butuh keputusan user)** | **Sweep mill flake: 1–2 test/run kena `Test timeout of 20000ms exceeded`** (butuh 21–30,6 s) sementara hasil scan tetap `OK 0 critical/serious` → budget timeout 20 s tidak cukup di environment ini; TIDAK terkait perubahan OPS-06 (`Home.tsx` + edge, bukan rute mill) | 3 run berturut 2026-09-24 (`.agents/logs/gate-ops06-gateB.log`, `gate-ops06-mill.log`, `gate-ops06-mill3.log`): run1 = 1 failed + 1 flaky, run2 = 2 failed + 1 flaky, run3 = 2 failed (semua pesan `Test timeout of 20000ms exceeded`, 0 a11y violation); dev server lama dibersihkan + cold start tetap sama | Sweep `-g mill` hijau 3× berturut pada budget saat ini, ATAU naikkan `timeout` per-test di `playwright.a11y-sweep.config.ts` dengan alasan tertulis. Bukti: 3 run exit 0 | OPEN |
