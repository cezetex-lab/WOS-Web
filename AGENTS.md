...
| OPS-08 | **P3** | **a11y suite terblokir rate limiter OTP saat run beruntun** — `hit_rate_limit` (migrasi 193, `pwreset_login_otp`) max 3/NRP per 15 menit. Suite a11y makan 2-3 OTP per run (Admin + Dashboard login admin) → 2 run beruntun terblokir. Bukan bug: limiter bekerja benar. | DB: `rate_limits` rows `{identifier:"NRP101", action:"pwreset_login_otp", count:3, window_start:...}` (helper 2026-09-22). Snapshot: "Terlalu banyak request OTP. Coba lagi nanti." | Runner a11y wajib **rotate NRP admin** per test (mis. NRP100, NRP101, NRP102...) ATAU tambah delay 15 menit antar run ATAU tambah opsi `--no-rate-limit-check`. Bukti: 3 run a11y beruntun < 5 menit = 0 OTP block. | OPEN |
| OPS-09 | **P2** | **a11y sweep 154 halaman: 36 halaman violation (55 node: 23 critical + 32 serious).** Pola: color-contrast 25 test (Tailwind bg-teal-500/bg-blue-500/bg-sky-600 + teks putih, rasio 2.48–3.67 < 4.5), select-name 8 test (18 node, `<select>` tanpa accessible name), label 2 test (5 node), scrollable-region-focusable 1 test. Plus 3 timeout worker (/worker, /worker/continuous-perf, /worker/fatigue — networkidle, bukan a11y) + 1 flaky (/worker/safety). | Sweep 2026-09-22: 116/154 PASS, 36 violation test, 3 timeout. Bukti: `.agents/logs/a11y-sweep-result2.json` (7,5 MB) + `sweep-final-rows.json` (parsed); screenshot + `*-axe-violations.json` per test di `test-results/a11y/`. | Fix color-contrast dulu (keputusan user: Opsi B) ✅ apply commit e3ca82d — 24 lokasi/19 file (Button shared forms.tsx + cards badge + ChatCopilot + per-page); gate types/lint 0 error, regresi a11y 6 halaman 6/6 PASS 0 violation. **EFEK KE SWEEP 154 HALAMAN: BELUM DIUKUR** (re-sweep ditunda keputusan user 2026-09-22; draft "12 violation" adalah proyeksi, bukan hasil ukur). Command dipisah: `npm run test:a11y` (6 halaman) vs `test:a11y:full` (sweep 154). Sisa: fix select-name (8), label (2), scrollable (1); timeout 3 investigasi terpisah. Bukti dampak penuh = re-run `npm run test:a11y:full`. | OPEN (color-contrast fix ter-apply, dampak sweep belum terbukti) |
