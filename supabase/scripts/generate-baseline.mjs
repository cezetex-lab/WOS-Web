...
emit(`--   * Partisi \`hr_attendance_*\` tidak di-hardcode di sini: dipanggil`);
emit(`--     \`ensure_attendance_partitions()\` agar tidak berakhir seperti loop`);
emit(`--     2024..2027 di migrasi 141 (AGENTS.md §5.8 SQL-08).`);
...
emit('');
emit(`-- ── PARTISI ABSENSI (dinamis) — WAJIB SEBELUM ACL ───────────────`);
emit(`-- Partisi hr_attendance_YYYY_MM dibuat oleh fungsi ini, sedangkan bagian ACL di`);
emit(`-- bawah memberi GRANT langsung ke setiap partisi. Karena itu pemanggilannya harus`);
emit(`-- DI SINI — setelah FUNCTIONS (fungsinya sudah ada) dan sebelum ACL (partisinya`);
emit(`-- sudah harus ada). Sebelumnya blok ini dikirim di akhir berkas sehingga baseline`);
emit(`-- gagal dengan: relation "public.hr_attendance_2024_01" does not exist.`);
emit(`-- Idempoten: pemanggilan berulang hanya menambah partisi yang belum ada.`);
emit(`SELECT public.ensure_attendance_partitions('2024-01-01'::date,`);
emit(`  (EXTRACT(YEAR FROM CURRENT_DATE + interval '24 months')::int * 12`);
emit(`   + EXTRACT(MONTH FROM CURRENT_DATE + interval '24 months')::int)`);
emit(`  - (2024 * 12 + 1) + 1);`);
emit('');
...
emit('');
emit(`-- SAPUAN SCHEMA-WIDE (wajib, karena partisi tidak bisa didaftar di atas):`);
emit(`-- partisi hr_attendance_YYYY_MM dibuat SAAT INSTALASI oleh`);
emit(`-- ensure_attendance_partitions(), sehingga namanya belum ada ketika berkas ini`);
emit(`-- di-generate. Tanpa sapuan ini setiap partisi baru mewarisi hak bawaan platform`);
emit(`-- (anon ALL) → DB perusahaan baru LEBIH TERBUKA daripada DB live.`);
emit(`-- Terukur pada uji replay 2026-09-18: 48 entri ACL berlebih pada partisi.`);
...
emit('');
emit(`-- Catatan: partisi absensi sudah dibuat lebih awal (blok "PARTISI ABSENSI"),`);
emit(`-- karena ACL per-partisi membutuhkannya. Jendela partisi tetap dihitung dari`);
emit(`-- waktu berjalan sehingga instalasi tidak pernah kehabisan partisi (§5.8 SQL-08).`);
emit('');
