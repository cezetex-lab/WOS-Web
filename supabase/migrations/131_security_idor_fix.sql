...
-- Problem: Worker RPCs accept ANY p_nrp from client
-- Fix: Validate p_nrp matches caller's own NRP (or admin/owner bypass)
-- ============================================================

-- Helper: Get caller's NRP from auth.uid()
...
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    SELECT 1 FROM user_roles WHERE nrp = _get_caller_nrp() AND role IN ('owner', 'admin_pusat', 'admin_hrd', 'admin_finance', 'admin_produksi', 'admin_mining', 'admin_mill', 'admin_estate')
  ) OR EXISTS (
...
-- ============================================================
-- Fix get_worker_profile: validate p_nrp = caller's NRP
-- ============================================================
...
DECLARE
  ...
  v_role RECORD;
BEGIN
  -- IDOR FIX: Only allow own data unless admin/owner
  IF NOT v_is_admin AND p_nrp != v_caller THEN
    RETURN jsonb_build_object('ok', false, 'msg', 'Akses ditolak: hanya bisa melihat data sendiri.');
  END IF;
  ...
  RETURN jsonb_build_object('ok', true, 'nrp', v_emp.nrp, 'nama', v_emp.nama, 'nik', v_emp.nik,
    'email', v_emp.email, 'divisi', v_emp.divisi, 'posisi', v_emp.posisi,
    'level', COALESCE(v_role.role_level, 1), 'tier', COALESCE(v_role.plan, 'FREE'),
    'tanggal_lahir', v_emp.tanggal_lahir, 'jenis_kelamin', v_emp.jenis_kelamin,
    'status_kerja', v_emp.status_kerja, 'tanggal_mulai', v_emp.tanggal_masuk, 'no_hp', v_emp.no_hp);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- ============================================================
-- Fix get_worker_status: validate p_nrp = caller's NRP
-- ============================================================
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) as total INTO v_att
  FROM hr_attendance WHERE nrp = p_nrp
    AND date >= date_trunc('month', NOW())
    AND date < date_trunc('month', NOW()) + INTERVAL '1 month';

  RETURN jsonb_build_object('ok', true,
    'kpi_score', COALESCE(v_kpi.kpi_score, 0), 'kpi_period', COALESCE(v_kpi.periode, ''),
    'attendance_hadir', COALESCE(v_att.hadir, 0), 'attendance_telat', COALESCE(v_att.telat, 0),
    'attendance_izin', COALESCE(v_att.izin, 0), 'attendance_total', COALESCE(v_att.total, 0));
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- ============================================================
-- Fix get_worker_payroll: validate p_nrp = caller's NRP
-- ============================================================
...
BEGIN
  IF NOT v_is_admin AND p_nrp != v_caller THEN
    RETURN jsonb_build_object('ok', false, 'msg', 'Akses ditolak: hanya bisa melihat gaji sendiri.');
  END IF;
...
-- ============================================================
-- Fix get_worker_leave: validate p_nrp = caller's NRP
-- ============================================================
...
BEGIN
  ...
  RETURN (
    SELECT jsonb_build_object('ok', true, 'data', COALESCE(jsonb_agg(row_to_json(t)), '[]'::jsonb))
    FROM (SELECT * FROM hr_leave WHERE nrp = p_nrp ORDER BY tahun DESC LIMIT 5) t
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- ============================================================
-- Fix get_worker_engagement: validate p_nrp = caller's NRP
-- ============================================================
...
-- ============================================================
-- Fix get_worker_learning: validate p_nrp = caller's NRP
-- ============================================================
...
BEGIN
  ...
  RETURN (
    SELECT jsonb_build_object('ok', true, 'data', COALESCE(jsonb_agg(row_to_json(t)), '[]'::jsonb))
    FROM (SELECT * FROM hr_learning WHERE nrp = p_nrp ORDER BY created_at DESC LIMIT 20) t
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- ============================================================
-- Fix get_worker_notifications: validate p_nrp = caller's NRP
-- ============================================================
...
BEGIN
  ...
  RETURN (
    SELECT jsonb_build_object('ok', true, 'data', COALESCE(jsonb_agg(row_to_json(t)), '[]'::jsonb))
    FROM (SELECT * FROM hr_notifications WHERE nrp = p_nrp ORDER BY created_at DESC LIMIT 20) t
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- ============================================================
-- Fix get_worker_benefits: validate p_nrp = caller's NRP
-- ============================================================
CREATE OR REPLACE FUNCTION get_wo

-- Fix get_worker_benefits
...
-- Fix submit_voice
CREATE OR REPLACE FUNCTION submit_voice(p_type TEXT, p_nrp TEXT, p_title TEXT, p_details TEXT, p_anonymous BOOLEAN DEFAULT TRUE)
RETURNS JSONB AS $$
DECLARE v_caller TEXT := _get_caller_nrp(); v_is_admin BOOLEAN := _is_admin_or_owner_caller();
BEGIN
  IF NOT v_is_admin AND p_nrp != v_caller THEN RETURN jsonb_build_object('ok', false, 'msg', 'Akses ditolak.'); END IF;
