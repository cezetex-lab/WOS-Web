...
CREATE OR REPLACE FUNCTION get_worker_profile(p_text TEXT) RETURNS JSONB AS $$
DECLARE v_caller TEXT := _get_caller_nrp(); v_is_admin BOOLEAN := _is_admin_or_owner_caller(); v_emp RECORD; v_role RECORD;
BEGIN
  IF NOT v_is_admin AND p_text != v_caller THEN RETURN jsonb_build_object('ok',false,'msg','Akses ditolak.'); END IF;
...
CREATE OR REPLACE FUNCTION get_worker_status(p_text TEXT) RETURNS JSONB AS $$
...
CREATE OR REPLACE FUNCTION get_worker_payroll(p_text TEXT) RETURNS JSONB AS $$
...
CREATE OR REPLACE FUNCTION get_worker_leave(p_text TEXT) RETURNS JSONB AS $$
DECLARE v_caller TEXT := _get_caller_nrp(); v_is_admin BOOLEAN := _is_admin_or_owner_caller(); 
BEGIN
  IF NOT v_is_admin AND p_text != v_caller THEN RETURN jsonb_build_object('ok',false,'msg','Akses ditolak.'); END IF;
...
CREATE OR REPLACE FUNCTION get_worker_engagement(p_text TEXT) RETURNS JSONB AS $$
...
CREATE OR REPLACE FUNCTION get_worker_learning(p_text TEXT) RETURNS JSONB AS $$
DECLARE v_caller TEXT := _get_caller_nrp(); v_is_admin BOOLEAN := _is_admin_or_owner_caller(); 
BEGIN
  IF NOT v_is_admin AND p_text != v_caller THEN RETURN jsonb_build_object('ok',false,'msg','Akses ditolak.'); END IF;
...
CREATE OR REPLACE FUNCTION get_worker_notifications(p_text TEXT) RETURNS JSONB AS $$
DECLARE v_caller TEXT := _get_caller_nrp(); v_is_admin BOOLEAN := _is_admin_or_owner_caller(); 
BEGIN
  IF NOT v_is_admin AND p_text != v_caller THEN RETURN jsonb_build_object('ok',false,'msg','Akses ditolak.'); END IF;
...
CREATE OR REPLACE FUNCTION get_worker_benefits(p_text TEXT) RETURNS JSONB AS $$
DECLARE v_caller TEXT := _get_caller_nrp(); v_is_admin BOOLEAN := _is_admin_or_owner_caller(); 
BEGIN
  IF NOT v_is_admin AND p_text != v_caller THEN RETURN jsonb_build_object('ok',false,'msg','Akses ditolak.'); END IF;
...
CREATE OR REPLACE FUNCTION get_worker_skills(p_text TEXT) RETURNS JSONB AS $$
DECLARE v_caller TEXT := _get_caller_nrp(); v_is_admin BOOLEAN := _is_admin_or_owner_caller(); 
BEGIN
  IF NOT v_is_admin AND p_text != v_caller THEN RETURN jsonb_build_object('ok',false,'msg','Akses ditolak.'); END IF;
...
CREATE OR REPLACE FUNCTION get_worker_overtime(p_text TEXT) RETURNS JSONB AS $$
DECLARE v_caller TEXT := _get_caller_nrp(); v_is_admin BOOLEAN := _is_admin_or_owner_caller(); 
BEGIN
  IF NOT v_is_admin AND p_text != v_caller THEN RETURN jsonb_build_object('ok',false,'msg','Akses ditolak.'); END IF;
...
CREATE OR REPLACE FUNCTION get_worker_medical(p_text TEXT) RETURNS JSONB AS $$
DECLARE v_caller TEXT := _get_caller_nrp(); v_is_admin BOOLEAN := _is_admin_or_owner_caller(); 
BEGIN
  IF NOT v_is_admin AND p_text != v_caller THEN RETURN jsonb_build_object('ok',false,'msg','Akses ditolak.'); END IF;
