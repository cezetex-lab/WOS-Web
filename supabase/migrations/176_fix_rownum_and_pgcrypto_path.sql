...
SELECT 'VERIFY: create_worker_request callable (pgcrypto reachable)' AS test,pg_temp.smoke_check($smoke$SELECT (create_worker_request('TEST','LEAVE','test','test') IS NOT NULL)$smoke$) AS result;

SELECT 'VERIFY: get_simper_list callable' AS test,pg_temp.smoke_check($smoke$SELECT (get_simper_list('TEST') IS NOT NULL)$smoke$) AS result;
SELECT 'VERIFY: get_heavy_equipment callable' AS test,pg_temp.smoke_check($smoke$SELECT (get_heavy_equipment('TEST') IS NOT NULL)$smoke$) AS result;
SELECT 'VERIFY: get_fatigue_data callable' AS test,pg_temp.smoke_check($smoke$SELECT (get_fatigue_data('TEST') IS NOT NULL)$smoke$) AS result;
SELECT 'VERIFY: get_production_daily callable' AS test,pg_temp.smoke_check($smoke$SELECT (get_production_daily('TEST') IS NOT NULL)$smoke$) AS result;
SELECT 'VERIFY: get_safety_incidents callable' AS test,pg_temp.smoke_check($smoke$SELECT (get_safety_incidents('TEST') IS NOT NULL)$smoke$) AS result;
SELECT 'VERIFY: get_jsa_list callable' AS test,pg_temp.smoke_check($smoke$SELECT (get_jsa_list('TEST') IS NOT NULL)$smoke$) AS result;
