...
## [2026-09-23] OPS-10 SELESAI — /admin/mill timeout root cause 3-layer (role mismatch + flash-redirect + churn)

- **Investigasi (read-only):** RPC `get_industry_admin_stats('mill')` 128–183 ms (identik antar industri) → kategori "data besar/query lambat" TERBANTAHKAN; komponen 4,2 KB tanpa chart. Probe browser (storageState admin): URL akhir `/admin` — **admin_hrd di-redirect oleh `useAdminAuth`** dari `/admin/mill` (`useAdminAuth.ts:21` menolak sebelum role termuat), heading mill tak pernah muncul; churn network: `get_branding` 4 call (jeda 0ms/1588ms/8ms, ada 2 paralel) + token refresh 5,6 dtk → jendela networkidle intermiten tersapu (PASS 13:12, timeout 21:33, PASS 21:44).
- **Fix test-side (Opsi A-Custom user):** storageState khusus role admin_mill — generator `setup-auth-state.cjs` di-extend (blok login NRP105 via akun.txt, OTP dev-mode; kredensial dari `loadMillFromAkun()` di kedua jalur return) → `auth-admin-mill.json` (3412 byte) terbentuk; sweep spec: `/admin/mill` di-extract ke describe terpisah `test.use({ storageState: AUTH['admin-mill'] })` (describe admin difilter `!== '/admin/mill'`); `settle()` networkidle 8s → 15s.
- **Bukti:** sweep mill-only `-g "mill"` = **8/8 PASS (2.0m)** — `admin /admin/mill` 9.6s, 0 critical/serious, TANPA timeout; smoke 6-page `npm run test:a11y` = 6/6 PASS (1.3m); gate types+lint 0 error. Catatan: judul describe tidak boleh mengandung "mill" (grep `-g` match judul penuh → sebelumnya tak sengaja memicu 100 test).
- **OPS-10 CLOSED** (Dipangkas 2026-09-23). Akar app-side di-register: **OPS-11** (get_branding dedupe, P3, OPEN) + **OPS-12** (useAdminAuth loading state sebelum navigate, P3, OPEN).
- Rate limiter tidak disentuh; tidak ada provisioning password; 0 file `src/`.
- **Dampak lintas-page: worker → admin → dashboard → owner** — 0 file `src/` (test + script only); scan `/admin/mill` kini benar-benar scan halaman mill (role admin_mill), halaman lain tidak berubah perilaku; `settle()` 15s berlaku ke semua test sweep (estimasi +7s/test worst-case, masih dalam budget 20s).
## [2026-09-24] OPS-12 SELESAI — useAdminAuth roleSettled (fail-safe 8s): cegah flash-redirect deep-link/refresh

- **Fix (commit `5db8a85`, 1 file `src/hooks/useAdminAuth.ts`, +17/−2):** redirect `/admin` pada hook kini dicekal sampai role termuat ATAU fail-safe habis. State `roleSettled` (useState) + 2 useEffect: (1) role ada → settle segera; role kosong → timer 8000 ms lalu settle; (2) guard redirect diawali `if (!roleSettled) return;` — deps `[role, roleSettled, navigate, allowedRoles]`.
- **Fail-safe 8 dtk (bukan 3) — keputusan user:** komentar di kode "Fail-safe 8 dtk (bukan 3) — token refresh bisa 5,6 dtk saat jaringan lambat; jangan bounce user authorized." (kalibrasi: token refresh tercatat 5,6 dtk pada probe OPS-10).
- **Kontrak return `{ role, isAllowed }` TIDAK berubah** — `git grep -l "useAdminAuth(" src/ | wc -l` = **50** file pemakai, semua memanggil inline dan hanya mengonsumsi role/isAllowed → aman.
- **Gate (working tree = commit):** `npm run check:types` (tsc --noEmit) EXIT 0; `npm run lint` (eslint src/ tests/) EXIT 0; `npm test` = **22 files / 141 passed** (40,13 dtk).
- **Bukti bukti flash-redirect hilang (deterministik, probe `.agents/scripts/ops12-nobounce-probe.cjs` — login NRP102/admin_finance via UI 1 OTP dev-mode, storageState `auth-admin-finance.json` 3424 byte, deep-link `/admin/payroll`, halaman lazy `Payroll` allowed `["admin_pusat","admin_finance"]`):**
  - SEBELUM (stash fix sementara = kode HEAD lama, REPRO delay penulisan sessionStorage `wos_user_v2` 3000 ms via addInitScript): URL akhir **`http://localhost:5173/`** — **BOUNCE**, h1 "Payroll" TIDAK MUNCUL.
  - SESUDAH (fix aktif, mekanisme repro identik): URL akhir **`http://localhost:5173/admin/payroll`** — **TIDAK BOUNCE**, h1 "Payroll" MUNCUL.
  - Tanpa REPRO (jaringan cepat lokal): kode lama pun tidak bounce (race tidak sempat terjadi) — menegaskan bug bersifat race saat restore sesi lambat (deep-link/refresh + provisioning), persis pola bukti live TASK 3 OPS-10 (admin_hrd URL akhir `/admin`, heading mill 0) dan token refresh 5,6 dtk.
- **Regresi:** `npm run test:a11y` = **6/6 PASS** (1.4m, 0 violation); sweep mill `-g "mill"` = **8/8 PASS** (1.6m, `/admin/mill` role admin_mill 11.1s tanpa bounce — OPS-10 tetap sehat); `npm run test:e2e -- tests/e2e/four-page-smoke.spec.ts` = **4/4 PASS** (47.5s; Worker, Admin, Dashboard, Owner — 2 OTP NRP101, dalam limit 3/15 menit).
- **Catatan investigasi:** `src/components/AdminRouteGuard.tsx` punya redirect serupa tapi **0 importer** (dead component) — bukan sumber bounce, tidak diubah.
...
## [2026-09-24] OPS-06 SELESAI — sync auth.users.password + reset_required flow fix

- **Perubahan (commit `768c683`, push `ecde223..768c683 migrasi-vite -> migrasi-vite` exit 0):**
  - Edge `password-reset`: action baru `change_password_sync` — RPC `change_password` tetap source of truth (worker_passwords + invalidasi sesi + audit), lalu `updateUserById` service role menyinkronkan `auth.users`; rate limit 5/15 mnt per NRP; response `auth_synced`. Otorisasi = `old_password` valid.
  - `supabase/scripts/record-password.mjs` (BARU, dipindah dari `.agents/scripts/` ke `supabase/scripts/` sesuai §0.11): update kolom password aktif di `supabase/akun/akun.txt` per NRP; opsi `--from-env NAMA_VAR` membaca dari `.env.local` sehingga password tidak masuk argv/output.
  - `Home.tsx`: submit reset password via edge `change_password_sync` (sebelumnya `rpc('change_password')` yang **tidak di-grant anon** → gagal diam-diam) + banner "Copy" password baru (untuk dicatat ke `akun.txt`) + tombol submit kedua "Lanjut Masuk" (login ulang) alih-alih ganti password lagi.
- **Temuan sampingan:** RPC `change_password` tidak di-grant anon → alur `reset_required` di `Home.tsx` (berjalan sebelum sesi ada) selama ini gagal diam-diam; diperbaiki lewat edge. **Catatan scope:** `admin_reset_worker_password` (dipakai `src/features/platform/auth/ResetPassword.tsx:88`) TIDAK diubah pada sesi ini — sinkronya tetap mengandalkan self-heal B1a′ (OPS-04) saat fast path berikutnya; belum diuji manual untuk jalur admin.
- **Bukti manual NRP002 (script `.agents/scripts/ops06-manual-test.mjs a|b|c`; password/hash tidak dicetak, hanya hasil pencocokan `crypt()`):**
  - **R0 recovery (PASS):** `setBoth: ok`; `worker_passwords crypt(E2E)=hash → true | reset_required → false`; `auth.users crypt(E2E)=hash → true`; `signInWithPassword(E2E) → OK 200`.
  - **R1 skenario (PASS):** `edge HTTP: 200 | {"ok":true,"msg":"Password berhasil diubah.","auth_synced":true}`; `signInWithPassword(DUMMY baru) → OK 200 | signInWithPassword(E2E lama) → GAGAL (400)`; `worker_passwords crypt(DUMMY)=hash → true | crypt(E2E)=hash → false | reset_required → false`; `auth.users crypt(DUMMY)=hash → true | crypt(E2E)=hash → false`.
  - **R1 cleanup round-trip via edge (PASS):** `edge HTTP: 200 | auth_synced: true`; `worker_passwords crypt(E2E)=hash → true | crypt(DUMMY)=hash → false | reset_required → false`; `auth.users crypt(E2E)=hash → true | crypt(DUMMY)=hash → false`; `signInWithPassword(E2E) → OK 200 | signInWithPassword(DUMMY) → GAGAL (400)`.
  - **akun.txt updated (masked):** `node supabase/scripts/record-password.mjs --nrp NRP002 --from-env E2E_WORKER_PASS` → `OK: password NRP002 diperbarui`; verifikasi `baris (masked): NRP002 | nrp002@insightwos.internal | 3204000000000002 |  | ***` + `kolom password aktif idx: 4 | len: 16 | cocok E2E_WORKER_PASS: true`. Berkas `supabase/akun/akun.txt` gitignored (`git check-ignore` → true), tidak ikut commit.
...
- **Apply:** `apply-migration.mjs --apply` → `status : DITERAPKAN + terdaftar + checksum terverifikasi (1515ms)` checksum `3bc1ec8f…`. Sequence pasca-apply: `role_permission_sets max(id)=28/seq=28`, `user_role_assignments max(id)=34/seq=34`.
  ...
  - Semua kegagalan = `Test timeout of 20000ms exceeded` (durasi aktual 21–30,6 s) dengan hasil scan `OK 0 critical/serious`; TIDAK terkait perubahan ini (tidak menyentuh rute mill/a11y spec). Raw: `.agents/logs/gate-ops06-gateB.log`, `gate-ops06-mill.log`, `gate-ops06-mill3.log`. §0.15 #6: berhenti setelah 3 percobaan, lapor.
- **Secret scan (R2 step 1):** `git diff --cached | findstr /R "postgresql:// SERVICE_ROLE password secret token api_key"` → 61 baris match, **seluruhnya IDENTIFIER-ONLY** (`old_password`, `new_password`, `change_password_sync`, nama variabel/komentar); 0 match nilai rahasia (`postgresql://`, `SUPABASE_SERVICE_ROLE_KEY=<nilai>`, JWT `eyJ...`, `sk-...`) → ABORT tidak terpenuhi.
- **Chain §0.3:** COMMIT `768c683` → PUSH `ecde223..768c683 migrasi-vite -> migrasi-vite` (exit 0) → Vercel auto-deploy push-triggered: alias `https://insightwos.vercel.app` HTTP **200**; edge `password-reset` ter-deploy (probe `change_password_sync` LIVE).
